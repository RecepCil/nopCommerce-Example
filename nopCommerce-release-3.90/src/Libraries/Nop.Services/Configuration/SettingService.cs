    ...
    public partial class SettingService : ISettingService
    {
        ...
        public virtual T LoadSetting<T>(int storeId = 0) where T : ISettings, new()
        {
            ...
            foreach (var prop in typeof(T).GetProperties())
            {
                ...
                if (!prop.CanRead || !prop.CanWrite)        // propertyleri okuyup yazabiliyorsak devam et yoksa continue ile atla
                    continue;

                var key = typeof(T).Name + "." + prop.Name;
                //load by store
                var setting = GetSettingByKey<string>(key, storeId: storeId, loadSharedValueIfNotFound: true);  // tekrar bak!!!
                if (setting == null)
                    continue;
