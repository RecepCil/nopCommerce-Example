...
namespace Nop.Services.Catalog
{
    ...
    public partial class ProductService : IProductService
    {
        ...
        public virtual void DeleteProducts(IList<Product> products)
        {
            ...
            //delete product
            UpdateProducts(products);           // Deleted bool'u true yapýlmýþ product'ý güncelle

            foreach (var product in products)       // Anlamadým, loglama mý?
            {
                //event notification
                _eventPublisher.EntityDeleted(product);
            }
        ...
        public virtual IList<Product> GetProductsByIds(int[] productIds)
        {
            ...
            var query = from p in _productRepository.Table              // Ýlgili tablonun property'leri
                        where productIds.Contains(p.Id) && !p.Deleted
                        select p;
            var products = query.ToList();                              // ilgili database satýrý
            ...
            return sortedProducts;                                      // ilgili database satýrý
        }
